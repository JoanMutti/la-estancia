      ...
      <ul className="navbar-nav me-auto mb-2 mb-lg-0">
        {navOptions.map((type) => (
          <li className="nav-item" key={type.key + 42}>
            <Link className="btn btn-outline-light border-0 rounded-pill m-1" to={type.key !='productos' ? '/productos/' + type.key: '/productos'} onClick={()=>{setType(products.filter((p)=>p.type.includes(type.key)))}}>
              {type.value}
            </Link>
          </li>
        ))}
